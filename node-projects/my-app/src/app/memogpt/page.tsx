...
// Removed import of speakBrowser to use global script instead
...
export default function Component() {
  ...
  const chatGPTRequest = async (message: string) => {
    try {
      ...
      if (
        ...
        typeof (window as Window & { speakBrowser?: (text: string) => void })
          .speakBrowser === 'function'
      ) {
        (window as Window & { speakBrowser?: (text: string) => void }).speakBrowser!(
          reply
        );
      }
      return reply;
  ...
  return (
    <>
      <Script src="/js/speakBrowser.js" strategy="beforeInteractive" />
      <div className="flex flex-col md:flex-row min-h-screen">
        {/* Left Sidebar */}
        <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-gray flex flex-row md:flex-col">
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="max-w-4xl border-2 border-primary bg-black">
            <DialogHeader className="border-b border-gray pb-4">
            ...
            <div className="max-w-4xl mx-auto">
              <div className="relative">
                <Textarea
                  ...
                  disabled={isLoading}
                  className="w-full h-12 md:h-14 px-4 md:px-6 pr-28 md:pr-32 text-base md:text-lg border-2 border-primary rounded-full focus:border-primary/80 field-sizing-content"
                />
                <Button
                  onClick={handleSubmit}
                  size="icon"
                  className="absolute right-14 top-1/2 transform -translate-y-1/2 h-9 w-9 md:h-10 md:w-10 rounded-full bg-primary hover:bg-primary/80 disabled:bg-gray-300 disabled:hover:bg-gray-300"
                  title={isSpeechOn ? 'Disable speech' : 'Enable speech'}
                  aria-label={isSpeechOn ? 'Disable speech' : 'Enable speech'}
                  onClickCapture={(e) => {
                    e.stopPropagation();
                    toggleSpeech();
                  }}
