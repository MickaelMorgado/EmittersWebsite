...
export default function TradingBotDashboard() {
  ...
  const fetchTrades = useCallback(async () => {
    try {
      const res = await fetch('/api/mika-bot');
  ...
  const fetchReportHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/mika-bot/report-history');
  ...
  useEffect(() => {
    ...
    try {
      ...
      tradesSource = new EventSource('/api/mika-bot/stream');
      ...
      positionsSource = new EventSource('/api/mika-bot/positions');
  ...
  const generateReport = async (tradeCount: number) => {
    ...
    try {
      const response = await fetch('/api/mika-bot/report', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        setTimeout(() => {
          fetch('/api/mika-bot/report-history', { method: 'POST' })
