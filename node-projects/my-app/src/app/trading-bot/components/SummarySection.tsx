      ...
      {aiAnalysis ? (
            <div className="bg-gradient-to-br from-violet-500/[0.08] to-purple-500/[0.03] border border-violet-500/[0.12] p-4 rounded">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-violet-300 uppercase tracking-wider">Live Signal</span>
                </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs uppercase text-white/30 tracking-wider">Trend</span>
                ...
                {aiAnalysis.patterns.length > 0 && (
                  <div>
                    <span className="text-xs uppercase text-white/30 tracking-wider">Patterns</span>
                    <div className="flex gap-1.5 flex-wrap mt-1">
                      {aiAnalysis.patterns.map((p: string, i: number) => (
                        <span key={i} className="text-xs px-2 py-0.5 bg-white/[0.05] text-white/50 border border-white/[0.06]">{p}</span>
                      ))}
                    </div>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3 pt-1">
                  <div>
                    <span className="text-xs uppercase text-white/30 tracking-wider">RSI</span>
                    <div className="text-base font-bold font-mono text-white/80 mt-0.5">{aiAnalysis.rsi?.toFixed(1) || 'N/A'}</div>
                  </div>
                  <div>
                    <span className="text-xs uppercase text-white/30 tracking-wider">Confidence</span>
                    <div className="text-base font-bold font-mono text-violet-400 mt-0.5">{aiAnalysis.confidence}%</div>
                  </div>
                </div>
                {aiAnalysis.reasoning && (
                  <div className="pt-2 border-t border-white/[0.05]">
                    <span className="text-xs uppercase text-white/30 tracking-wider">Reasoning</span>
            ...
            <div className="col-span-6 bg-gradient-to-br from-cyan-500/[0.08] to-blue-500/[0.03] border border-white/[0.05] p-4 rounded">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-white/70 uppercase tracking-wider">Session Summary</span>
              </div>
              <div className="grid grid-cols-2 gap-6 mb-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-cyan-300/80 uppercase tracking-wider">Profitability</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                    <span className="text-xs font-bold text-cyan-300/80 uppercase tracking-wider">Trade Analysis</span>
                  </div>
